...
export default async function getCroppedImg(
  ...
  // If we have a target size, use a second canvas to perform the resize
  if (targetSize) {
    ...
    // First put original crop into a temp canvas
    const tempCanvas = document.createElement("canvas");
    tempCanvas.width = pixelCrop.width;
    tempCanvas.height = pixelCrop.height;
    tempCanvas.getContext("2d")?.putImageData(data, 0, 0);

    // Draw temp canvas into resize canvas (interpolating)
    resizeCtx.drawImage(tempCanvas, 0, 0, targetSize.width, targetSize.height);

    return new Promise((resolve) => {
      resizeCanvas.toBlob((file) => resolve(file), "image/jpeg", 0.9);
    });
  }

  // Fallback if no target size (just crop)
